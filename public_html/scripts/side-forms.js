...
function addSide(parent, addSideIndex) {
    ...
    if (addSideIndex === 0) {
        parent.before(side);
        parent.querySelector('input[type="button"]').hidden = true;
    } else {
        parent.after(side);
    }
...
function renumberSideForms(addOrRemove, currentSideIndex) {
    ...
    let newNextSideIndex;
    let legend;
    let newNextSideLegendNumber;
    if (addOrRemove === 1) {
        ...
        for (let side = currentSideIndex + 1; side < sidesCount; side++) {
            ...
            newNextSideIndex = side + 1;
            newNextSideLegendNumber = newNextSideIndex + 1;
            //Меняем номер стороны (fieldset) в data-index
            sideForms[side].dataset.index = newNextSideIndex;
            //Меняем номер в легенде
            legend = sideForms[side].querySelector('legend');
            legend.innerText = legend.innerText.replace(/\d+$/, newNextSideLegendNumber);
            ...
            for (let i = 0; i < formTagsCount; i++) {
                if (formTags[i].hasAttribute('id')) {
                    formTags[i].id = formTags[i].id.replace(/\d+$/, newNextSideIndex);
                }
                if (formTags[i].hasAttribute('name')) {
                    formTags[i].name = formTags[i].name.replace(/\d+$/, newNextSideIndex);
                }
                if (formTags[i].hasAttribute('for')) {
                    formTags[i].for = formTags[i].for.replace(/\d+$/, newNextSideIndex);
                }
                if (formTags[i].classList.contains('side_form_legend')) {
                    formTags[i].innerHTML = formTags[i].innerHTML.replace(/\d+$/, newNextSideIndex);
                }
            }
        }
