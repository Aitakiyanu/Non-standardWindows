...
<fieldset>
    ...
    <input type="number" id="wall_opening_side_width_{$i}" name="wall_opening_side_width_{$i}" value="{$wallOpeningSides[$i]->sideWidth}" min="0" required>
    <label for="wall_opening_side_height_{$i}">высота по вертикали:</label>
    <input type="number" id="wall_opening_side_height_{$i}" name="wall_opening_side_height_{$i}" value="{$wallOpeningSides[$i]->sideHeight}" min="0" required><br/><br/>
